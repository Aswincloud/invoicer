...
export function renderInvoicePdf(inv, items, totals) {
  ...
  //
  // Drawn as filled rectangles rather than embedded images. This writer has no
  // image support beyond the signature's 1-bit mask, and a QR is nothing but a
  // grid of black squares, so vectors avoid that machinery entirely and stay
  // sharp at any zoom or print resolution.
  //
  // Runs of adjacent dark modules collapse into ONE rectangle. A 33x33 code is
  // 1089 modules; emitted individually that is ~44KB of content stream on every
  // invoice, and merging typically cuts it by two thirds for free.
  const drawQr = (matrix, x, topY, box) => {
    const mod = box / (matrix.length + QR_QUIET * 2);
    for (let r = 0; r < matrix.length; r++) {
      ...
      while (c < matrix.length) {
        ...
        p.rect(x + (c + QR_QUIET) * mod, topY - (r + QR_QUIET + 1) * mod,
               run * mod, mod, "0 0 0");
        c += run;
      }
    }
  ...
  if (payQr || orderQr) {
    ...
    // Captions are load-bearing with two codes on one page: an unlabelled pair
    // is a coin flip for whoever is holding it.
    const label = (x, text) =>
      p.text(x, qTop - BOX - 2, fit(text, BOX + GAP - 2, 7.5),
             { size: 7.5, color: "0.36 0.39 0.45" });

    if (payQr) {
      drawQr(payQr, qx, qTop, BOX);
      label(qx, "Scan to pay by UPI");
      qx += BOX + GAP;
    }
    if (orderQr) {
      drawQr(orderQr, qx, qTop, BOX);
      label(qx, String(inv.qr_caption || "").trim() || "Scan to order online");
    }
  }
