...
check("no phone is FINE (they just get no WhatsApp receipt)", validatePayForm(env, { ...good, phone: "" }).ok && validatePayForm(env, { ...good, phone: "" }).phone === "");
check("but a phone that IS given must be valid", !validatePayForm(env, { ...good, phone: "12345" }).ok);
check("no 'what' is fine", validatePayForm(env, { ...good, what: "" }).ok);
check("no address is fine", validatePayForm(env, { ...good, address: "" }).ok);
check("but an address that IS given must look like one", !validatePayForm(env, { ...good, address: "Chennai" }).ok);
check("its error mentions the PIN code and the blank option", /PIN/.test(validatePayForm(env, { ...good, address: "Chennai" }).error) && /blank/.test(validatePayForm(env, { ...good, address: "Chennai" }).error));
check("name + amount alone is a complete form", validatePayForm(env, { name: "Priya R", amount: "350" }).ok);
